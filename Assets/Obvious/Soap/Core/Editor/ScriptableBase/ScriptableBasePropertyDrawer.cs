...
namespace Obvious.Soap.Editor
{
    ...
    public abstract class ScriptableBasePropertyDrawer : PropertyDrawer
    {
        ...
        protected void DrawIfNull(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var isCollection = SoapTypeUtils.IsCollection(propertyType);
            ...
            else
            {
                ...
                if (fieldInfo != null)
                {
                    ...
                    if (GUI.Button(rect, guiContent))
                    {
                        ...
                        var tag = GetTagFromAttribute(soapSettings);
                        if (CanBeSubAsset)
                            CreateSubAsset(property, tag);
                        else
                            CreateScriptableObjectAtPath(property, tag);
                    }
                }
            }
        ...
        private void CreateScriptableObjectAtPath(SerializedProperty property, int tagIndex)
        {
            var soapSettings = SoapEditorUtils.GetOrCreateSoapSettings();
            ...
            var scriptable = SoapEditorUtils.CreateScriptableObjectAt(fieldInfo.FieldType, newName, path);
        ...
        private int GetTagFromAttribute(SoapSettings soapSettings)
        {
