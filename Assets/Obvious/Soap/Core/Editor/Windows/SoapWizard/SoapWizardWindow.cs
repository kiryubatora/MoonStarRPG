    ...
    public class SoapWizardWindow : EditorWindow
    {
        ...
        private void CheckShortcutInputs()
        {
            ...
            if (_currentEvent.type == EventType.KeyDown && _scriptableObjects.Count > 0)
            {
                ...
                if (increment != 0)
                {
                    ...
                    Deselect();
                    var isInFavoriteTab = _currentType == ScriptableType.Favorite;
                    var scriptableObjects = isInFavoriteTab ? Favorites : _scriptableObjects;
                    _selectedScriptableIndex = (selected + increment + scriptableObjects.Count) %
                                               scriptableObjects.Count;
                    _scriptableBase = scriptableObjects[_selectedScriptableIndex];
                    Repaint();
                }
            }
        ...
        private void DrawScriptableBases(List<ScriptableBase> scriptables)
        {
            ...
            void DrawScriptable(int i)
            {
                ...
                // Add favorite icon
                _stringBuilder.Clear();
                if (Favorites.Contains(scriptable))
                    _stringBuilder.Append("\u2605 ");
        ...
        private void Refresh(ScriptableType type)
        {
            switch (type)
            {
                ...
                case ScriptableType.Favorite:
                    ...
                    _subAssetsLookup = SoapEditorUtils.GetSubAssets(Favorites);
                    break;
            }
        }
